...
serve(async (req) => {
  ...
  try {
    ...
    if (!tokenCache || now >= tokenCache.expiresAt - 120000) {
      ...
      tokenCache = { accessToken: bbData.access_token, expiresAt: now + (bbData.expires_in * 1000) };
    }

    const pixRes = await fetch(`https://api.hm.bb.com.br/pix/v2/cob/${txid}?gw-dev-app-key=${Deno.env.get("BB_APP_KEY")}`, {
      method: "PUT",
      headers: { 
    ...
    await supabase.from("pix_charges").update({ pix_copia_cola: pixData.pixCopiaECola }).eq("txid", txid);

    return new Response(JSON.stringify({ success: true, ...pixData }), { 
      status: 200, 
