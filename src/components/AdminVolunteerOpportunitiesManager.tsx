...
interface AdminVolunteerOpportunitiesManagerProps {
  nucleus: string;
}
...
export function AdminVolunteerOpportunitiesManager({ nucleus }: AdminVolunteerOpportunitiesManagerProps) {
  const queryClient = useQueryClient();
  const [dialogOpen, setDialogOpen] = useState(false);
  const [selectedOpportunityId, setSelectedOpportunityId] = useState<string | null>(null);
  const [editingOpportunity, setEditingOpportunity] = useState<any>(null);
  const [form, setForm] = useState({ title: '', description: '', image_url: '', time_commitment: '', status: 'ativo' as string, max_slots: '' });

  const { data: opportunities, isLoading } = useQuery({
    queryKey: ['admin-volunteer-opportunities', nucleus],
    queryFn: async () => {
  ...
  const { data: registrations, isLoading: registrationsLoading } = useQuery({
    queryKey: ['volunteer-opportunity-registrations', selectedOpportunityId],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    mutationFn: async (opp: { title: string; description: string; nucleus: string; image_url?: string; time_commitment?: string; status: 'ativo' | 'encerrado'; max_slots?: number }) => {
      const { error } = await supabase.from('volunteer_opportunities').insert([opp]);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, ...opp }: any) => {
      const { error } = await supabase.from('volunteer_opportunities').update(opp).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-volunteer-opportunities'] });
  ...
  const updateRegistrationMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      const { error } = await supabase.from('volunteer_opportunity_registrations').update({ status }).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['volunteer-opportunity-registrations'] });
      toast({ title: 'Status atualizado!' });
    },
  });

  const resetForm = () => {
    setForm({ title: '', description: '', image_url: '', time_commitment: '', status: 'ativo', max_slots: '' });
  ...
  const handleSubmit = () => {
    ...
    const payload: { title: string; description: string; nucleus: string; status: 'ativo' | 'encerrado'; image_url?: string; time_commitment?: string; max_slots?: number } = {
    ...
    if (form.time_commitment) payload.time_commitment = form.time_commitment;
    if (form.max_slots) payload.max_slots = parseInt(form.max_slots);

    if (editingOpportunity) {
      updateMutation.mutate({ id: editingOpportunity.id, ...payload });
    } else {
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="icon" onClick={() => setSelectedOpportunityId(null)}>
              <ArrowLeft className="h-4 w-4" />
            </Button>
            <div>
              <CardTitle>Inscritos - {selectedOpp?.title}</CardTitle>
        ...
        <Dialog open={dialogOpen} onOpenChange={(open) => { if (!open) resetForm(); setDialogOpen(open); }}>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-4 mt-2">
              <div>
                <Label>Título</Label>
                <Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Nome da oportunidade" />
              </div>
              <div>
              ...
              <div>
                ...
                <Input value={form.time_commitment} onChange={(e) => setForm({ ...form, time_commitment: e.target.value })} placeholder="Ex: 4h por semana" />
              </div>
              <div>
